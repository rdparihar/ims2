...
class StockClosingView(generic.ListView):
    model = Shop
    context_object_name = 'shop_list'
    template_name = 'Stock/closing.html'
    def get_context_data(self, *args, **kwargs):
        context = super(StockClosingView, self).get_context_data(*args, **kwargs)
        id = self.request.session.get('userid')
        isSubAdmin = self.request.session['isSubAdmin']
        isAdmin = self.request.session['isAdmin']
        isSuperUser = self.request.session.get('isSuperUser')
        isUser = self.request.session['isUser']

        if isAdmin or isSuperUser:
            # context['shop'] = Shop.objects.filter(shop_id=1)
            context['shop_list'] = self.model.objects.all()
        else:
            if isSubAdmin:
                context['shop_list'] = self.model.objects.filter(shop_admin = id)
            elif isUser:
                context['shop_list'] = self.model.objects.filter(shop_keeper = id)
            else:
                context['shop_list'] = ""

        return context

class StockPurchaseView(generic.ListView):
    model = Shop
    context_object_name = 'shop_list'
    template_name = 'Stock/purchase.html'
    def get_context_data(self, *args, **kwargs):
        context = super(StockPurchaseView, self).get_context_data(*args, **kwargs)
        id = self.request.session.get('userid')
        isSubAdmin = self.request.session['isSubAdmin']
        isAdmin = self.request.session['isAdmin']
        isSuperUser = self.request.session.get('isSuperUser')
...
def update_content(dsreport_date,ds_shop_id):
    ...
    for category in category_rows:
        ...
        for brand in brand_rows:
            ...
            stock_opens = StockOpen.objects.all().filter(brand_id=brand_id,open_shop_id=ds_shop_id,open_date=dsreport_date).values()
            ...
            for quantity in Quantity_Master:
                stock_receipts = Invoice.objects.all().filter(brand_id=brand_id,shop_id=ds_shop_id,invoice_date=dsreport_date,invoice_brand_size=quantity).values('invoice_brand_qty').first()
                if stock_receipts:
                    wtsheet.write(row_num, 9+qty_cnt, stock_receipts['invoice_brand_qty'], receipt_qty_style)
                    qty_cnt=qty_cnt+1

            stock_closes = StockClose.objects.all().filter(brand_id=brand_id,close_shop_id=ds_shop_id,close_date=dsreport_date).values()
...
def dsreport(request):
    if request.method =="POST":
        ...
        ds_shop_id = request.POST['ds_shop_id']
        update_content(dsreport_date, ds_shop_id)
